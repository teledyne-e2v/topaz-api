...
int64_t get_control_max(char *name)
{
    ...
    return control_list.controls[i].maximum;
}
    ...
    { // Parse the file to get information
        ...
        if (index > 2 && strlen(splited[1]) > 2)
        {
            if (strncmp(splited[1], "0x", 2) == 0)
            {
                ...
                if (strncmp(control_list.controls[j].name, "sensor", 6) != 0)
                    set_control_by_code(control_list.controls[j].id, control_list.controls[j].default_value);
                j++;
            }
        }
    }
